...
```R
#  the image being used in this example is titled 'A tiger in the water'
#  By Bob Jagendorf 
...
> tiger$set_brightness(.5) #sets brightness by number inputted
> tiger$set_brightness(0) #sets brightness back to default
```

#### Contrast
```R
# There are three ways to modify contrast
# The initial value of contrast is 1
...
> tiger$set_contrast(1) #sets contrast back to default
```
...
> tiger$set_gamma(1) #sets gamma correction back to default
...
> tiger$set_blur(6) #sets blurring by number inputted
> tiger$set_blur(0) #sets blurring back to default
...
> tiger$set_rotate(180) #sets rotation by degree inputted
> tiger$set_rotate(0) #sets rotation back to default of 0 degrees rotated
...
> tiger$gethistory() #returns recorded changes to image
> tiger$get_brightness() #returns brightness value
> tiger$get_contrast() #returns contrast value
> tiger$get_gamma() #returns gamma correction value
> tiger$get_blur() #returns blur value
> tiger$get_rotate() #returns degrees rotated
> tiger$get_color() #returns color mode value
```
...
# A user can also use an image they are currently editing in the GUI
> runShiny(tiger)
```
